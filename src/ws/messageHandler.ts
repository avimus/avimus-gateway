...
function toQueuedMessage(messageId: string, tenantId: string, message: ApplicationMessage): QueuedMessage {
...
export async function handleApplicationMessage(
  ...
  if (deps.queue.hasQueued(tenantId)) {
    ...
    deps.queue.enqueue(tenantId, toQueuedMessage(messageId, tenantId, message));
  ...
  try {
    await deliverLive(tenantId, message, deps);
    sendMessage(connection.socket, { type: "ack", messageId, status: "received" });
  } catch (err) {
